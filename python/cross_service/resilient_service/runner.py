...
class Runner:
    ...
    def deploy(self):
        ...
        print(
            "Creating and populating a DynamoDB table named '{self.recommendation.table_name}'."
        )
        ...
        if not lb_success:
            ...
            if not port_is_open:
                ...
                if q.ask(
                    "Do you want to add a rule to security group {sec_group['GroupId']} to allow\n"
                    "inbound traffic on port {self.port} from your computer's IP address of {current_ip_address}? (y/n) ",
                    q.is_yesno,
                ):
            ...
            if not ssh_port_is_open:
                if q.ask(
                    "Do you want to add a rule to security group {sec_group['GroupId']} to allow\n"
                    "inbound SSH traffic on port {self.ssh_port} for debugging from your computer's IP address of {current_ip_address}? (y/n) ",
                    q.is_yesno,
                ):
                    self.autoscaler.open_inbound_port(
